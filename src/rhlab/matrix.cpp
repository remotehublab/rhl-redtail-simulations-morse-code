...
void MatrixSimulation::print_gpio_header_states(){
    this->log() << "----- Input -----" << endl;
    for(int i = 0; i < this->getNumberOfSimulationInputs(); i++){
...
void MatrixSimulation::print_buffer_states(){
    this->log() << "buffer: ";
    for(int i = 0; i < BUFFER_ARRAY_SIZE; i++){
...
void MatrixSimulation::print_led_states(){
...
bool MatrixSimulation::read_literal_logic(string s){
    return s[0] == 'T';
}

// Converts ST or SF to a boolean True or False
bool MatrixSimulation::read_switch_logic(string s){
    return s[0] == 'T';
}

// From a string that provdes the gpio index (i.e. g02), obtain the current GPIO value
bool MatrixSimulation::read_gpio_logic(string s){
    int index = stoi(s);
...
void MatrixSimulation::update_gpio_logic(string s, bool o){
    int index = stoi(s);
...
bool MatrixSimulation::read_buffer_logic(string s){
    int index = stoi(s);
    return this->buffer[index];
}

// From a string that provides the buffer index (i.e. b0), set the current buffer value
void MatrixSimulation::update_buffer_logic(string s, bool o){
    int index = stoi(s);
    this->buffer[index] = o;
}

// From a string that provides the led index (i.e. d0), set the current led value
void MatrixSimulation::update_led_logic(string s, bool o){
    int index = stoi(s);
...
//       - Switch (i.e. ST or SF)
int MatrixSimulation::read_gate_input(string substring){
    int in;
    if(substring == "L"){
        return IS_LITERAL;
    }
...
int MatrixSimulation::read_gate_output(string substring){
    int out;
    if(substring == "g"){
        return IS_GPIO;
    }
...
bool MatrixSimulation::handle_input(string substring, int &start_index){
    int p;
...
void MatrixSimulation::handle_output(string substring, int &start_index, bool logic_gate_output){
    int p;
...
int MatrixSimulation::read_logic_gate(string substring){
    int start_index = 0;
...
int FPGA_DE1SoC_MatrixSimulation::getNumberOfSimulationInputs() {
    return 7;
}

int FPGA_DE1SoC_MatrixSimulation::getNumberOfSimulationOutputs() {
    return 5;
}

int STM32_WB55RG_MatrixSimulation::getNumberOfSimulationInputs() {
    return 6;
}

int STM32_WB55RG_MatrixSimulation::getNumberOfSimulationOutputs() {
    return 5;
}
